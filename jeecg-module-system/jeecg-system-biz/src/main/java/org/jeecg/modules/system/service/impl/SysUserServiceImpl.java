...
//		this.save(user);  //保存角色的时候已经添加过一次了
    ...
    public LoginUser getEncodeUserInfo(String username) {
        ...
        // 查询当前登录用户的部门id
        SysDepart sysDepart = this.getDepartByOrgCode(sysUser.getOrgCode());
        // 查询当前登录用户的部门id
        if (sysDepart != null) {
            loginUser.setOrgId(sysDepart.getId());
            loginUser.setOrgName(sysDepart.getDepartName());
            SysDepart sysCompany = this.getCompanyByOrgInfo(sysDepart);
            if (sysCompany != null) {
                loginUser.setCompanyCode(sysCompany.getOrgCode());
                loginUser.setCompanyName(sysCompany.getDepartName());
                loginUser.setCompanyId(sysCompany.getId());
            }
        }
...
//		}
    ...
    private @Nullable SysDepart getDepartByOrgCode(String orgCode) {
        if (oConvertUtils.isEmpty(orgCode)) {
            return null;
        }
        LambdaQueryWrapper<SysDepart> queryWrapper = new LambdaQueryWrapper<>();
    ...
    private @Nullable SysDepart getCompanyByOrgInfo(SysDepart sysDepart) {
        //TODO 查询组织机构信息速度需要优化
        if ("1".equals(sysDepart.getOrgCategory())) {
            return sysDepart;
        }
        while (true) {
            LambdaQueryWrapper<SysDepart> queryWrapper = new LambdaQueryWrapper<>();
            queryWrapper.eq(SysDepart::getId, sysDepart.getParentId());
            sysDepart = sysDepartMapper.selectOne(queryWrapper);
            if (sysDepart == null || "1".equals(sysDepart.getOrgCategory())) {
                break;
            }
        }
        return sysDepart;
	...
	@Override
	public SysUser getUserByNameJoinMerchant(String userName) {
		return baseMapper.selectUserByNameJoinMerchant(userName);
	}
