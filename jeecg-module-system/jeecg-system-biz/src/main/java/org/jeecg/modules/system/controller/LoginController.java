...
//		redisUtil.del(realKey);
	...
	@RequestMapping(value = "/merchant_login", method = RequestMethod.POST)
	public Result<JSONObject> merchantLogin(@RequestBody SysLoginModel sysLoginModel, HttpServletRequest request){
		Result<JSONObject> result = new Result<JSONObject>();
		String username = sysLoginModel.getUsername();
		String password = sysLoginModel.getPassword();
		if(isLoginFailOvertimes(username)){
...
//		}
		...
		queryWrapper.eq(SysUser::getUsername,username);
		SysUser sysUser = sysUserService.getOne(queryWrapper);*/
		SysUser sysUser = sysUserService.getUserByNameJoinMerchant(username);
		result = sysUserService.checkUserIsEffective(sysUser);
		if(!result.isSuccess()) {
			return result;
		}
...
//		redisUtil.del(realKey);
	...
	public Result<LoginUser> getLoginUserInfo(HttpServletRequest request) {
