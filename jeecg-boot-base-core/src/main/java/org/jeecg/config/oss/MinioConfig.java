...
public class MinioConfig {
    ...
    @Value(value = "${jeecg.minio.minio_view_url}")
    private String minioViewUrl;
    ...
    public void initMinio() {
        ...
        if (!minioViewUrl.startsWith(CommonConstant.STR_HTTP)) {
            minioViewUrl = "http://" + minioViewUrl;
        }
        if (!minioViewUrl.endsWith(SymbolConstant.SINGLE_SLASH)) {
            minioViewUrl = minioViewUrl.concat(SymbolConstant.SINGLE_SLASH);
        }
        MinioUtil.setMinioUrl(minioUrl);
        MinioUtil.setMinioViewUrl(minioViewUrl);
