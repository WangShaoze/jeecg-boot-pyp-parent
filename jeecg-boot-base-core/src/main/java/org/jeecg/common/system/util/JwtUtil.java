...
//	public static final long EXPIRE_TIME = (7 * 12) * 60 * 60 * 1000;
	public static final long EXPIRE_TIME = 2 * 60 * 60 * 1000; //实际有效时间是这个的两倍
	...
	public static String getUserSystemData(String key, SysUserCacheInfo user) {
		...
		// 替换为系统用户登录所使用的公司ID
		else if (key.equals(DataBaseConstant.COMPANY_ID) || key.equalsIgnoreCase(DataBaseConstant.COMPANY_ID_TABLE)) {
			if (user == null) {
				returnValue = sysUser.getCompanyId();
			} else {
				returnValue = user.getCompanyId();
			}
		}

		// 替换为系统用户登录所使用的公司code
		else if (key.equals(DataBaseConstant.COMPANY_CODE) || key.equalsIgnoreCase(DataBaseConstant.COMPANY_CODE_TABLE)) {
			if (user == null) {
				returnValue = sysUser.getCompanyCode();
			} else {
